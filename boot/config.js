...
function loadConfig(logger, paths) {
    const config = {
        ...
        INFLUX_URL: process.env.INFLUX_URL?.trim() || '',
        INFLUX_TOKEN: process.env.INFLUX_TOKEN?.trim() || '',
        INFLUX_ORG: process.env.INFLUX_ORG?.trim() || '',
        INFLUX_BUCKET: process.env.INFLUX_BUCKET?.trim() || '',
        INFLUX_MEASUREMENT: process.env.INFLUX_MEASUREMENT?.trim() || 'korelate_events',
        INFLUX_BATCH_SIZE: process.env.INFLUX_BATCH_SIZE ? parseInt(process.env.INFLUX_BATCH_SIZE, 10) : 1000,
        INFLUX_BATCH_INTERVAL_MS: process.env.INFLUX_BATCH_INTERVAL_MS ? parseInt(process.env.INFLUX_BATCH_INTERVAL_MS, 10) : 2000,
