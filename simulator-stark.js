 ...
 *
 * Simulation Scenario: Stark Industries
 *
 * This module exports a factory function that creates a new
 * instance of the Stark Industries simulation.
 */
const spBv10Codec = require('sparkplug-payload').get("spBv1.0");

/**
 * Factory function for the Stark Industries simulation scenario.
...
module.exports = (logger, publish, isSparkplugEnabled) => {
    ...
    function publishBirthMessages() {
        ...
        logger.info("[StarkSim] Publishing Sparkplug NBIRTH messages for all devices...");
        ...
        logger.info("[StarkSim]    -> NBIRTH messages published (seq=0).");
    }
    ...
    const scenario = [
        ...
        // --- Fire alarm scenario ---
        // 7. Trigger alarm
        () => { logger.warn("[StarkSim] Injecting FIRE ALARM near Laser Welder."); return { topic: 'stark_industries/malibu_facility/fms/fire_alarm', payload: { zone: "Assembly Line 01 - Casing Cell", detector_id: "SMOKE-DET-1138", status: "ALARM" } }; },
        ...
        () => { logger.info("[StarkSim] Clearing Fire Alarm."); return { topic: 'stark_industries/malibu_facility/fms/fire_alarm', payload: { zone: "Assembly Line 01 - Casing Cell", detector_id: "SMOKE-DET-1138", status: "CLEAR" } }; },
        // 10. Resume work
        () => { simState.equipmentStatus.laser_welder_01.status = 'running'; return { topic: 'stark_industries/malibu_facility/assembly_line_01/vibranium_casing_cell/mes/operation', payload: { workOrderId: simState.workOrders.vibraniumCasing.id, step: 10, stepName: "Laser welding", operator: simState.operators[1], status: "IN_PROGRESS" } }; },
        // --- End fire scenario ---
        ...
        // 16. Inject fault
        () => { logger.warn("[StarkSim] Injecting fault into CNC Mill 05. High vibration data will start."); simState.equipmentStatus.cnc_mill_05.status = 'error'; return null; },
        ...
        // 18. Simulate "repair"
        () => { logger.info("[StarkSim] Fault on CNC Mill 05 is 'resolved'. Data will return to normal."); simState.equipmentStatus.cnc_mill_05.status = 'running'; return null; },
        // --- End incident ---
        ...
        () => { logger.warn("[StarkSim] Injecting NITROGEN LEAK. Pressure will drop rapidly."); simState.equipmentStatus.gas_tank_nitrogen.status = 'leaking'; return null; },
        ...
        () => { logger.info("[StarkSim] Nitrogen leak 'repaired'."); simState.equipmentStatus.gas_tank_nitrogen.status = 'online'; return null; },
        // --- End leak ---
    ...
    function start() {
        ...
        logger.info(`[StarkSim] Starting NARRATIVE loop. Publishing every ${NARRATIVE_INTERVAL_MS / 1000}s.`);
        ...
        logger.info(`[StarkSim] Starting SENSOR loop. Publishing every ${SENSOR_INTERVAL_MS / 1000}s.`);
    ...
    function stop() {
        ...
        if (narrativeInterval) {
            logger.info("[StarkSim] Stopping narrative loop.");
            clearInterval(narrativeInterval);
            narrativeInterval = null;
            stopped = true;
        }
        if (sensorInterval) {
            logger.info("[StarkSim] Stopping sensor loop.");
        ...
        logger.info("[StarkSim] All equipment statuses reset.");
        return stopped;
    }
